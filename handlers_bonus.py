...
async def bonus_receive(message: types.Message, state: FSMContext):
    ...
    caption = f"Бонус за отзыв от @{user.username or user.id}\nИмя: {user.first_name or ''}"
    if message.text:
        caption += f"\nСообщение: {message.text}"
    if message.photo:
        await message.bot.send_photo(CHANNEL_ID, message.photo[-1].file_id, caption=caption, message_thread_id=THREAD_ID)
    elif message.video:
        await message.bot.send_video(CHANNEL_ID, message.video.file_id, caption=caption, message_thread_id=THREAD_ID)
    elif message.document:
        await message.bot.send_document(CHANNEL_ID, message.document.file_id, caption=caption, message_thread_id=THREAD_ID)
    else:
